/**
 * Shared MCP server factory — used by stdio (Cursor) and HTTP (Claude.ai / ChatGPT / web).
 */
import { z } from "zod";
...
/** Register all compass-mcp tools on a fresh McpServer instance. */
export function createCompassMcpServer(): McpServer {
  const serverVersion = getServerVersion();
  ...
  server.tool(
    ...
    "MCP 새로고침 절차 (짧게). verbose=true면 전체 steps.",
    {
      host: refreshHostSchema.describe(
        "cursor | claude | openai | vscode | generic",
      ),
  ...
  server.tool(
    ...
    "명령 문장 정독 → 작업에 맞는 모델. compact JSON + run_hint(다음 Task model=primary_id). 에이전트: recommend 후 Task/subagent model=primary_id, log_model_usage, set_sticky. verbose=true면 scores.",
    {
      ...
      tags: z
        .array(tagSchema)
        .optional()
        .describe("ui | bug | architecture | test"),
      current_model: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    async ({ task_description, tags, locale }) => {
      const builtIn = verifyBuiltInScenarios(serverVersion);
  ...
  server.tool(
    ...
    async ({ category, verbose }) => {
      ...
      return jsonToolResult(
        { meta: EXAMPLE_PROMPTS_META, prompts: list },
        true,
      );
    },
  );
